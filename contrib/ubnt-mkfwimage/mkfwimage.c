...
static int write_part(void* mem, part_data_t* d)
{
	...
	if ((addr=(char*)mmap(0, d->stats.st_size, PROT_READ, MAP_SHARED,
	    fd, 0)) == MAP_FAILED)
	{
...
static void usage(const char* progname)
{
	INFO("Version %s\n"
             "Usage: %s [options]\n"
	     "\t-v <version string>\t - firmware version information, "
	        "default: %s\n"
...
static int validate_image_layout(image_info_t* im)
{
	...
	for (i = 0; i < im->part_count; ++i)
	{
		...
		if (d->stats.st_size > d->partition_length) {
			ERROR("File '%s' too big (%d) - max size: 0x%08X "
			    "(exceeds %lu bytes)\n",
...
static int build_image(image_info_t* im)
{
	...
	{
		part_data_t* d = &im->parts[i];
		mem_size += sizeof(part_t) + d->stats.st_size +
		    sizeof(part_crc_t);
	}
...
int main(int argc, char* argv[])
{
	...
	while ((o = getopt(argc, argv, OPTIONS)) != -1)
	{
		...
		case 'v':
			if (optarg)
				strncpy(im.version, optarg,
				    sizeof(im.version));
			break;
		case 'o':
			if (optarg)
				strncpy(im.outputfile, optarg,
				    sizeof(im.outputfile));
			break;
		case 'h':
			usage(argv[0]);
			return (-1);
		case 'k':
			if (optarg)
				strncpy(kernelfile, optarg,
				    sizeof(kernelfile));
			break;
		case 'r':
			if (optarg)
				strncpy(rootfsfile, optarg,
				    sizeof(rootfsfile));
			break;
		case 's':
			if (optarg)
