	...
	while ((c = getopt(argc, argv, "b:o:m:p:v:")) != -1) {
		switch (c) {
			...
			case 'b':			
				...
				trxtemp.offsets[nSegementCount++]=htole32(strtoul(optarg, &e, 0));
				if ((e == optarg) || *e) 
				{
...
#define ROUND 0x1000
	...
	p->magic = htole32(TRX_MAGIC);
	cur_len = sizeof(struct trx_header);
	p->flag_version = htole32((TRX_VERSION << 16));

	i = 0;

	while (optind < argc) {		
		/* jc */
		if(!le32toh(p->offsets[i])) 
		{			
			p->offsets[i] = htole32(cur_len);
		}
		else
		{			
			if(cur_len>le32toh(p->offsets[i]))
			{
				fprintf(stderr, "offset too large\n");
				return EXIT_FAILURE;			
			}
			cur_len=le32toh(p->offsets[i]);
		}
		/* jc end */
...
#define ROUND 0x1000
	...
	p->crc32 = htole32(p->crc32);

	p->len = htole32(cur_len);
