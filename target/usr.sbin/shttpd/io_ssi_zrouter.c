...
enum { SSI_PASS, SSI_BUF, SSI_EXEC, SSI_CALL, SSI_SYSCTL, SSI_KENV };
...
static void
do_sysctl(struct ssi *ssi, char *buf, int len, int *n)
{
	struct ssi_inc	*inc = ssi->incs + ssi->nest;
	char		cmd[sizeof(inc->buf)], *e, *p;
	size_t		l = len;
	...
	if (inc->cond == SSI_STOP) {
		...
		_shttpd_elog(E_LOG, ssi->conn, "ssi: bad sysctl(%s)", p);
	} else if (!_shttpd_url_decode(p + 1, e - p - 1, cmd, sizeof(cmd))) {
		_shttpd_elog(E_LOG, ssi->conn,
		    "ssi: cannot url_decode: sysctl(%s)", p);
	} else if (sysctlbyname(cmd, buf, &l, 0, 0)) {
		_shttpd_elog(E_LOG, ssi->conn, "ssi: sysctlbyname(%s, buf, &l, 0, 0), l = %d", cmd, l);
	} else {
		inc->state = SSI_SYSCTL;
		*n += l - 1;
		//do_sysctl2(ssi, buf, len, n);
		inc->state = SSI_STOP;
	}
}

static void
do_kenv(struct ssi *ssi, char *buf, int len, int *n)
{
	struct ssi_inc	*inc = ssi->incs + ssi->nest;
	char		cmd[sizeof(inc->buf)], *e, *p;
	size_t		l = len;
	...
	if (inc->cond == SSI_STOP) {
		...
		_shttpd_elog(E_LOG, ssi->conn, "ssi: bad kenv(%s)", p);
	} else if (!_shttpd_url_decode(p + 1, e - p - 1, cmd, sizeof(cmd))) {
		_shttpd_elog(E_LOG, ssi->conn,
		    "ssi: cannot url_decode: kenv(%s)", p);
	} else if ((l = kenv(KENV_GET, cmd, buf, l)) == -1 ) {
		_shttpd_elog(E_LOG, ssi->conn, "ssi: kenv(%s)", cmd);
	} else {
		inc->state = SSI_KENV;
		*n += l - 1;
		inc->state = SSI_STOP;
...
again:
	...
	if (inc->state == SSI_CALL)
		...
		do_exec2(ssi, buf, len, &n);
//	else if (inc->state == SSI_SYSCTL)
//		do_sysctl2(ssi, buf, len, &n);
//	else if (inc->state == SSI_KENV)
//		do_kenv2(ssi, buf, len, &n);
